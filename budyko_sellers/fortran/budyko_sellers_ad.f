...
C 
      ...
      subroutine f_ad( xxs, xxs_ad, j, j_ad )
C******************************************************************
C******************************************************************
...
C OPEN TAPE tapex
C----------------------------------------------
      open(unit=60,file='TAF_tape_1_f_t',ACCESS='DIRECT',RECL=n*8)
...
C----------------------------------------------
      t_ad(n) = t_ad(n)-0.5*j_ad
      t_ad(51) = t_ad(51)+0.5*j_ad
      t_ad(50) = t_ad(50)+0.5*j_ad
      t_ad(1) = t_ad(1)-0.5*j_ad
      j_ad = 0.d0
      do iter = max_iter, 1, -1
C$taf INCOMPLETE alpha
        ...
        do i = 1, n
          ...
          sx_ad(i) = sx_ad(i)+fin_ad(i)*(1.d0-alpha(i))
          fin_ad(i) = 0.d0
        end do
        do i = 1, n
          if (t(i) .le. t1) then
...
C==============================================
      ...
      j = 0.5*(t(50)+t(51))-0.5*(t(1)+t(n))
      open(unit=10,file='latitude.txt')
      open(unit=11,file='temperature.txt')
      open(unit=12,file='albedo.txt')
      open(unit=13,file='F_in.txt')
      open(unit=14,file='F_out.txt')
      open(unit=15,file='F_diff.txt')
      open(unit=16,file='net_flux.txt')
      open(unit=17,file='J.txt')
      do i = 1, n
        write(unit=10,fmt=*) lat(i)
        write(unit=11,fmt=*) t(i)
        write(unit=12,fmt=*) alpha(i)
        write(unit=13,fmt=*) fin(i)
        write(unit=14,fmt=*) fout(i)
        write(unit=15,fmt=*) fdiff(i)
        write(unit=16,fmt=*) fin(i)-fout(i)+fdiff(i)
      end do
      write(unit=17,fmt=*) j
      close(unit=10)
      close(unit=11)
